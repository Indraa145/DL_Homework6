...
class Data_Helper(object):
    ...
    def __init__(self, path,training_set_size,step_num,batch_num,training_data_source,log_path):
        ...
        self.sn_list = [self.sn[step_num*i:step_num*(i+1)].to_numpy() for i in range(t1)]
        self.va_list = [self.va[step_num*i:step_num*(i+1)].to_numpy() for i in range(t2)]
        self.vn1_list = [self.vn1[step_num*i:step_num*(i+1)].to_numpy() for i in range(t3)]
        self.vn2_list = [self.vn2[step_num*i:step_num*(i+1)].to_numpy() for i in range(t4)]
        
        self.tn_list = [self.tn[step_num*i:step_num*(i+1)].to_numpy() for i in range(t5)]
        self.ta_list = [self.ta[step_num*i:step_num*(i+1)].to_numpy() for i in range(t6)]
        
        self.va_label_list =  [self.va_labels[step_num*i:step_num*(i+1)].to_numpy() for i in range(t2)]
